...
class userorder extends React.Component {
  ...
  getData() {
    ...
    Axios.get('http://54.153.48.19:3001/orderstatus')
      .then((res) => {
        const data = res.data;
  ...
  handleDelete = (event) => {
    ...
    if (event.target.value === 'Ordered') {
      Axios.defaults.withCredentials = true;
      Axios.defaults.headers.common['authorization'] = localStorage.getItem('token');
      Axios.post('http://54.153.48.19:3001/updateordercan', visitdata)
        .then((res) => {
  ...
  onClickButton = (event) => {
    ...
    Axios.defaults.withCredentials = true;
    Axios.defaults.headers.common['authorization'] = localStorage.getItem('token');
    Axios.post('http://54.153.48.19:3001/uorderdeets', visitdata)
      .then((res) => {
        console.log(res.status);
  ...
  finalFilter = (filData) => {
    ...
    Axios.defaults.withCredentials = true;
    Axios.defaults.headers.common['authorization'] = localStorage.getItem('token');
    Axios.post('http://54.153.48.19:3001/filteruorders', filData)
      .then((res) => {
        if (res.status === 200) {
